...
The better way is to use a _template_ that provides a way to use dynamically added data on a set HTML structure, preventing us from having to type out these monster strings of data and markup.
...
```javascript
commentNode.innerHTML = "<div class='some-class'> This is a comment \
and it's real long</div><p>" + someDynamicValue + "</p><p>" + someOtherValue + \
"</p>"
```
...
<article>
  ...
  <p>
    After careful consideration and a generous grant from NASA, our team has
    determined that a woodchuck could chuck five wood, with a standard deviation
    of +/- .37 wood.
  </p>
...
We'll start by building a template string as we have in the past, but instead of concatenation, we'll be using _interpolation_, which is a way to replace text in strings using special delimeters.
